...
def create_feedback(
    ...
    # Log audit trail with the authenticated user
...
# Temporary route for testing without authentication - will be removed in production
@router.post("/test", response_model=FeedbackRead, include_in_schema=False)
def create_feedback_test(feedback: FeedbackCreate, db: Session = Depends(get_db)):
    db_feedback = Feedback(**feedback.dict())
    db.add(db_feedback)
    db.commit()
    db.refresh(db_feedback)
    
    # For testing, we'll use the system user from the database
...
async def receive_sms_webhook(request: Request, db: Session = Depends(get_db)):
    ...
    try:
        ...
        # Log audit trail using system user from database
        system_user = db.query(User).filter(User.username == "system").first()
        if not system_user:
            # Fallback if system user doesn't exist
            class SystemUser:
